...
class FixController:
    ...
    async def get_positions():
        ...
        if user_fix in fixM.main_tasks:
            log.info(f"si existe la sesion de fix")
            getP = fixM.main_tasks[user_fix].application.rest.get_positions(
                cuenta)
    ...
    async def iniciar_socket_bot(url, user, account, accountFixId, puertows):
        ...
        try:
            ...
            response = requests.post(
                f"{url}/api/iniciar_fix/new", data=json.dumps(payload), headers=headers)
    ...
    async def iniciar_fix_new():
        ...
        target = "ROFX"
        settings = config_fix_settings(
            req_obj["user"]["puerto"], BeginString, id_fix, target, id_fix)
        print("pase settings")
        ...
        if iniciarSocketBot == False:
            return {"status": False}
        mainFix = MainTask(settings, target, id_fix, req_obj["user"]["password"],
                           req_obj["cuenta"], target, accountFixId, req_obj["user"]["puertows"])
        ...
        mainFix.application.rest = RofexAPI(
            user=id_fix, password=req_obj["user"]["password"], base_url=url_rest)
    ...
    async def iniciar_fix_m():
        ...
        login = False
        try:
            sesionesFix[id_fix] = main(
                settings, target, id_fix, req_obj["user"]["password"], req_obj["cuenta"])
            ...
            if int(req_obj["user"]["live"]) == 1:
                url_rest = req_obj["user"]["url_rest"]
            sesionesFix[id_fix].application.rest = RofexAPI(
                user=id_fix, password=req_obj["user"]["password"], base_url=url_rest)
            ...
            while True:
                if sesionesFix[id_fix].application.sessions[target]['connected'] != None:
                    ...
                    if sesionesFix[id_fix].application.sessions[target]['connected'] == True:
                        ...
                        login = True
                        sesionesFix[id_fix].application.triangulos[0] = fixManual(
                            sesionesFix[id_fix].application, id_fix, 0, req_obj["cuenta"], user_id)
                        sesionesFix[id_fix].application.triangulos[0].daemon = True
                        sesionesFix[id_fix].application.triangulos[0].start()
                        time.sleep(1)
                        break
                    else:
                        response = {"status": False, "id_fix": id_fix,
                                    "mgs": "datos incorrectos"}
    ...
    def get_securitys():
        ...
        if id_fix in fixM.main_tasks:
            ...
            if cuentaFix["live"] == 1:
                account_type = "live"
            today = date.today()
            securitys = mongo.db.securitys.find_one(
                {"date": str(today), "account_type": account_type})
            if securitys:
                UtilsController.guardar_security_in_fix(
                    securitys["data"], id_fix)
                response = {"securitys": str(securitys["data"]).replace(
                    "'", '"'), "date": str(datetime.now()), "status": True}
            ...
            response = {"securitys": str(securitys_data).replace(
                "'", '"'), "date": str(datetime.now()), "status": True}
